...
'''
METHOD USED TO TEST SEARLE DATASET WITH PATTERNNET DATASET
'''
@torch.no_grad()
def patternnet_generate_predictions(clip_model: CLIP, ref_names_list: List[str], pseudo_tokens: torch.Tensor,output_path: str) -> Dict[str, float]:
    '''
        we will provide a method that performs evaluation to each image of the dataset patternnet that exists in the PatternCom corpus
    '''
    patternnet_path='/home/thanasis/datasets/PatternNet'
     # Create data loader
    ...
    device = 'cuda'
    clip_model = clip_model.to(device)
    #READING THE PATTERNCOM QUERIES AND PERFORMING AN EVALUATION PER QUERY
    for query_file in listdir(Path(f"{patternnet_path}/PatternCom")):
        image_filenames = []
        attributes = []
        attribute_values = []
        with open(Path(f"{patternnet_path}/PatternCom/{query_file}"), newline='') as csvfile:
        ...
        attribute_values_unique=list(set(attribute_values))
        #getting unique images for each attribute
        images_unique=list(set(image_filenames))
        print(f'Current Attribute Evaluation: {attributes[0]}')
        predicted_features_list = []
        images=[]
        queries=[]
        for img in tqdm(ref_names_list):
            if(os.path.basename(img) in image_filenames):
                for attr in attribute_values_unique:
                    #creating each input caption
                    input_captions=[f'{attr} $']

                    images.append(img)
                    queries.append(f'{attr} $')

                    batch_tokens = pseudo_tokens[ref_names_list.index(img)].unsqueeze(0).to(device)
        ...
        predicted_features = torch.vstack(predicted_features_list)
        print(len(predicted_features_list))
        Path(output_path).mkdir(parents=True, exist_ok=True)
        dict_save = {}
        dict_save['feats'] = predicted_features.data.cpu().numpy()
        dict_save['path'] = images
        dict_save['query'] = queries
        with open(f"{output_path}/{attributes[0]}.pkl","wb") as f:
...
def main():
    ...
    parser.add_argument("--dataset", type=str, required=True, choices=['cirr', 'fashioniq', 'circo', 'imagenet','patternnet'],
                        help="Dataset to use")
    ...
    if args.eval_type == 'oti':
        ...
        suffix = f"{0.02}_{350}"
        experiment_path = (
            PROJECT_ROOT / 'data_search' / "oti_pseudo_tokens_15_knn" / args.dataset.lower() / args.split / f"{args.dataset.lower()}_{suffix}" / args.clip_model_name.replace('/', ''))
        if not experiment_path.exists():
        ...
        ema_oti_pseudo_tokens = torch.load(experiment_path / 'ema_oti_pseudo_tokens.pt', map_location=device)
        with open(experiment_path / 'image_names.pkl', 'rb') as f:
            ref_names_list = pickle.load(f)
    ...
    if args.dataset.lower() == 'fashioniq':
        ...
        for domain in domains:
            ...
            with open(f"{output_path}/{domain}.pkl","wb") as f:
                pickle.dump(dict_save,f)
    elif args.dataset.lower()=='patternnet':
        output_path = f"SEARLE_data/{args.dataset.lower()}-{args.clip_model_name.replace('/','')}"
        text_features = patternnet_generate_predictions(clip_model=clip_model,ref_names_list=ref_names_list, pseudo_tokens=ema_oti_pseudo_tokens,output_path=output_path)
