...
class PatternNetDomainLabels(Dataset):
	def __init__(self, input_filename, transforms, root=None):
		with open(input_filename, 'r') as f:
			lines = f.readlines()
		filenames = [line.strip() for line in lines]
	...
	def __getitem__(self, idx):
		if self.root is not None:
			img_path = '/home/thanasis/datasets/PatternNet'+str(self.images[idx][1:])
		else:
			img_path = str(self.images[idx])
		images = self.transforms(Image.open(img_path))
		labels = self.labels[idx]
...
class FashionIQDataset(Dataset):
	...
	def __init__(self, dataset_path: Union[Path, str], split: Literal['train', 'val', 'test'], dress_types: List[str],
		...
		for dress_type in dress_types:
			with open(dataset_path / 'json' / f'cap.{dress_type}.{split}.json') as f:
