...
from datasets import FashionIQDataset, CIRRDataset, CIRCODataset, ImageDomainLabels, PatternNetDomainLabels
from utils_searle import device
from encode_with_pseudo_tokens import encode_with_pseudo_tokens
from utils_searle import get_templates
import time
...
def oti_inversion(args):
	...
	PROJECT_ROOT=Path("/home/thanasis/code/Composed_IR-searle_map/SEARLE")
	...
	suffix = f"{args.learning_rate}_{args.oti_steps}"
	experiment_path = (
			PROJECT_ROOT / 'data_search' / f"oti_pseudo_tokens_{args.top_k}_knn" / args.dataset.lower() / args.split / f"{args.exp_name}_{suffix}" / args.clip_model_name.replace('/', ''))
	...
	# Define the dataset
	root = "/home/thanasis/code/Composed_IR-searle_map/SEARLE"
	if args.dataset.lower()== 'patternnet':
		full_dataset_path='/home/thanasis/datasets/PatternNet/patternnet.csv'
		dataset=PatternNetDomainLabels(full_dataset_path, root=full_dataset_path,transforms=clip_preprocess)
		loader = DataLoader(dataset, batch_size=128, shuffle=False, num_workers=8, pin_memory=True, drop_last=False)
		...
		dataset = CIRCODataset(full_dataset_path, args.split, 'relative', clip_preprocess)
		loader = DataLoader(dataset, batch_size=args.batch_size, collate_fn=collate_fn, num_workers=8)
	...
	for batch_idx, batch in enumerate(tqdm(loader)):
		...
		if batch_idx == 0:
			print(f"Time taken>>>>>>>> {(1/3600) * len(loader) * (time.time() - start)} mins")
