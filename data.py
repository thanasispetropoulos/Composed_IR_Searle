...
from open_clip.clip import tokenize
...
class CIRR(Dataset):
    ...
    def return_testdata(self, idx):
        if self.mode == 'caps':
                ...
                return ref_images, str(self.ref_imgs[idx]), target_cap, self.pairids[idx]
        else:
    ...
    def return_valdata(self, idx):
        if self.mode == 'caps' and not self.vis_mode:
            ...
            ref_images = self.transforms(Image.open(img_path))
            target_cap = self.target_caps[idx]                
            return ref_images, str(self.ref_imgs[idx]), str(self.target_imgs[idx]), target_cap                       
        else:
            tar_path = str(self.target_imgs[idx])
            img_path = os.path.join(self.root_img, tar_path)
            target_images = self.transforms(Image.open(img_path))
            return target_images, os.path.basename(img_path)
...
class FashionIQ(Dataset):
    ...
    def return_all(self, idx):
        ...
        return ref_images, target_images, str(self.target_imgs[idx]), str(self.ref_imgs[idx]), cap1, cap2
...
class CsvCOCO(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        actual_text_query = " ".join([query_class]+other_classes)
        actual_class =  query_class
        filename = str(self.images[idx])
        return image, region_image, actual_text_query, actual_class, filename
...
class ImageList(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.return_filename and self.is_labels:
...
class CsvDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        texts = tokenize([str(self.captions[idx])], truncate=True)[0]
